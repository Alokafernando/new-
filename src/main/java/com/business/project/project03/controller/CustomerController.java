...
import lk.ijse.gdse.project.bo.BOFactory;
...
import lk.ijse.gdse.project.view.tdm.CustomerTM;
...
public class CustomerController {
    ...
    CustomerBO customerBOm = (CustomerBO) BOFactory.getInstance().getBO(BOFactory.BOType.CUSTOMER);
    ...
    void deleteCustomer(ActionEvent event) throws SQLException, ClassNotFoundException {
        ...
        if (optionalButtonType.get() == ButtonType.YES) {
            boolean isDeleted = customerBOm.delete(customerId);
            if (isDeleted) {
                refreshPage();
    ...
    void saveCustomer(ActionEvent event) throws SQLException, ClassNotFoundException {
        ...
        if(isValidName && isValidAddress && isValidContact && isValidEmail){
            ...
            boolean isSaved = customerBOm.save(new CustomerDTO(customerID,name,address,contact,email));
            if (isSaved) {
                refreshPage();
    ...
    void updateCustomer(ActionEvent event) throws SQLException, ClassNotFoundException {
        ...
        if(isValidName && isValidAddress && isValidContact && isValidEmail){
            ...
            boolean isUpdated = customerBOm.update(new CustomerDTO(customerID,name,address,contact,email));

            //boolean isUpdated = customerBOm.update(customerDTO);
            if (isUpdated) {
                refreshPage();
    ...
    private void loadTableData() throws SQLException, ClassNotFoundException {
        ArrayList<CustomerDTO> customerDTOS = customerBOm.getAll();
        System.out.println("Fetched Data: " + customerDTOS);
    ...
    private void loadNextCustomerId() throws SQLException, ClassNotFoundException {
        String nextCustomerID = customerBOm.generateNewID();
